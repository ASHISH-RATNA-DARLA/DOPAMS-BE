-- ========================================================================
-- MATERIALIZED VIEW RECONSTRUCTION - CORRECTED VERSION
-- All critical issues from compatibility audit fixed
-- ========================================================================

-- Drop existing views if they exist to allow clean creation
...
-- ========================================================================
-- VIEW: accuseds_mv - CORRECTED
-- ========================================================================
-- FIXES APPLIED:
-- 1. Changed bfd.primary_drug_name to bfd.drug_name
-- 2. Changed bfd.weight_kg to bfd.standardized_weight_kg
-- 3. Changed bfd.volume_ml to bfd.standardized_volume_ml
-- 4. Changed bfd.count_total to bfd.standardized_count
-- 5. Removed non-existent a.accused_status reference
-- 6. Removed volume_l which doesn't exist in schema
...
SELECT 
  ...
  (
    SELECT COALESCE(
      ARRAY_AGG(DISTINCT UPPER(TRIM(bfd.drug_name))) FILTER (WHERE bfd.drug_name IS NOT NULL AND bfd.drug_name != 'NO_DRUGS_DETECTED'),
      ARRAY[]::text[]
    )
    FROM brief_facts_drug bfd WHERE bfd.crime_id = c.crime_id
  ) AS "drugType",
  (
    SELECT jsonb_agg(
      jsonb_build_object(
...
-- ========================================================================
-- VIEW: firs_mv - CORRECTED
-- ========================================================================
-- FIXES APPLIED:
-- Same drug column fixes as accuseds_mv
...
-- ========================================================================
-- VIEW: advanced_search_accuseds_mv - CORRECTED
-- ========================================================================
...
SELECT 
  ...
  (
    SELECT jsonb_agg(
      jsonb_build_object(
        'name', bfd.drug_name,
        'quantityKg', bfd.standardized_weight_kg,
        'quantityMl', bfd.standardized_volume_ml,
        'quantityCount', bfd.standardized_count,
...
-- ========================================================================
-- VIEW: advanced_search_firs_mv - CORRECTED
-- ========================================================================
...
-- ========================================================================
-- VIEW: criminal_profiles_mv - CORRECTED
-- ========================================================================
-- FIXES APPLIED:
-- 1. ADDED missing crimes JSONB field - CRITICAL FIX
-- 2. Used correct drug column names
...
SELECT 
  ...
  (SELECT COUNT(DISTINCT c.crime_id) FROM accused a INNER JOIN crimes c ON a.crime_id = c.crime_id WHERE a.person_id = p.person_id)::bigint AS "noOfCrimes",
  (
    SELECT COUNT(*)::bigint
    FROM accused a
    LEFT JOIN brief_facts_accused bfa ON a.accused_id = bfa.accused_id
    WHERE a.person_id = p.person_id AND UPPER(TRIM(COALESCE(bfa.status, 'Unknown'))) = 'ARRESTED'
  ) AS "arrestCount",
  (
...
LEFT JOIN brief_facts_drug bfd ON a.crime_id = bfd.crime_id
